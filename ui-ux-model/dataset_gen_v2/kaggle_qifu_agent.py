...
def load_remote_rag_sources(max_rows: int = 250) -> list[dict[str, str]]:
    ...
    enabled = {
        ...
        for name in os.getenv(
            "QIFU_REMOTE_RAG_SOURCES",
            "websight,screen2words,webui",
        ).split(",")
        if name.strip()
    }
    sources = [source for source in all_sources if source[0].lower() in enabled]
    print("Remote RAG sources:", ", ".join(source[0] for source in sources))
